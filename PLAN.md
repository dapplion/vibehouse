...
- **Test coverage** — in progress (21 Gloas fork choice unit tests + 17 bid processing unit tests + 17 envelope processing unit tests + 15 withdrawal processing unit tests + 15 builder pending payments epoch processing unit tests + 21 state upgrade unit tests + 13 block replayer unit tests + 27 builder deposit processing unit tests + 20 payload attestation & PTC committee unit tests + 18 PtcDutiesMap unit tests + 15 attestation participation flag unit tests + 42 gossip verification integration tests + 9 genesis initialization unit tests + 7 expected withdrawals phase unit tests + 6 per_slot_processing payload availability unit tests + 6 proposer slashing payment removal unit tests + 4 same-slot attestation weight unit tests + 10 verify_attestation committee index tests + 8 proto_array viability tests + 6 attestation signing payload_present tests + 11 fork choice node state transition tests + 13 signature set construction tests + 17 ForkChoice wrapper method tests + 5 Builder::is_active tests + 14 verify_committee_index Gloas attestation validation tests + 8 gossip verification error path tests + 6 envelope signature verification tests + 11 per-block Gloas orchestration + fork dispatch tests + 8 process_proposer_lookahead tests + 22 execution_payload_header Gloas conversion tests + 10 execution_payload Gloas SSZ/accessor tests + 18 payload Gloas Full/Blinded type tests + 42 DataColumnSidecar Gloas variant tests + 32 BuilderBid Gloas type tests + 35 BeaconBlockBody Gloas variant tests + 13 SignedBeaconBlock Gloas blinding/conversion tests + 9 engine API Gloas type & NewPayloadRequest tests + 21 PubsubMessage Gloas gossip encode/decode tests + 75 ePBS Gloas type behavioral tests + 25 BeaconStateGloas unit tests + 22 ChainSpec Gloas timing/domain/scheduling tests + 15 ForkName Gloas unit tests + 11 ProposerPreferences tests + 8 SignedProposerPreferences tests + 11 BuilderPendingPayment tests + 11 BuilderPendingWithdrawal tests + 4 PtcDutyData API serde tests + 15 ExecutionBidPool edge case tests + 6 ObservedExecutionBids edge case tests + 9 ObservedPayloadAttestations edge case tests + 17 Gloas beacon_chain integration tests + 7 attestation index validation tests + 17 should_extend_payload/get_payload_tiebreaker tests + 16 get_gloas_weight/should_apply_proposer_boost_gloas tests + 18 compute_filtered_roots/get_ancestor_gloas/is_supporting_vote_gloas/get_gloas_children tests + 12 find_head_gloas proposer boost/gloas_head_payload_status tests + 16 BeaconChain Gloas method integration tests + 8 import_payload_attestation_message integration tests + 9 Gloas HTTP API integration tests + 10 envelope/body type tests + 6 HTTP API envelope POST/pre-Gloas guard tests + 12 HTTP API proposer_lookahead/PTC duties/attestation/withdrawal tests + 24 SSE event & API type tests + 5 fork choice state verification tests + 5 execution proof chain-dependent integration tests + 6 process_epoch_single_pass Gloas integration tests + 6 block production payload attestation packing tests + 2 dependent root stability proof tests + 2 store cold state dual-indexing tests + 3 block verification bid/DA bypass tests + 7 envelope processing integration tests + 3 engine API Gloas wire format tests + 7 stateless validation execution proof threshold tests + 9 gossip verification error path tests + 16 Gloas slot timing tests + 7 execution payload path integration tests + 13 fork choice Gloas method integration tests + 6 validator store Gloas signing domain tests + 5 fork choice attestation import integration tests + 5 fork transition boundary integration tests + 2 import_execution_bid integration tests + 6 network gossip handler integration tests + 8 proposer preferences gossip handler tests + 6 block verification Gloas edge case tests + 5 attestation production payload_present tests + 6 gossip execution payload envelope handler tests + 6 execution proof gossip handler tests + 5 early attester cache Gloas payload_present tests + 8 self-build envelope EL/error path tests + 4 canonical_head/payload attributes Gloas branch tests + 6 execution bid gossip builder-path tests + 3 payload attestation gossip handler tests + 3 proposer preferences bid validation tests + 3 external builder block import lifecycle tests + 5 POST envelope error path/payload attestation data HTTP tests + 1 duplicate proposer preferences gossip test + 3 self-build envelope EL response tests + 10 withdrawal edge case tests + 4 multi-epoch chain health integration tests + 4 HTTP API builder bid submission tests + 3 envelope edge case integration tests + 3 sign_proposer_preferences validator store tests + 3 load_parent/range sync integration tests + 3 gossip envelope/load_parent EMPTY/historical attestation tests + 3 get_advanced_hot_state envelope re-application tests + 3 external builder envelope gossip verification tests + 3 pool/fork-choice field behavior tests + 3 gossip envelope EL error path/cross-epoch withdrawal tests + 3 canonical head head_hash fallback tests + 3 proposer boost timing/payload invalidation tests + 3 gossip verification/execution status lifecycle tests + 3 fork transition boundary/envelope error path tests + 3 bid/attestation equivocation and builder balance tests + 3 proposer preferences bid validation tests + 3 payload attestation gossip validation tests + 3 bid gossip validation tests + 3 bid signature/envelope finalization tests + 5 fork choice queued attestation dequeue/tiebreaker/multi-hop ancestor tests + 2 fork choice same-slot vote spec boundary tests + 5 state processing error path tests + 5 BuilderPubkeyCache unit tests + 5 EMPTY parent/is_parent_block_full/withdrawal cap tests + 7 EarlyAttesterCache Gloas payload_present tests + 4 update_builder_pubkey_cache tests + 3 withdrawal OOB builder_index/nonzero index tests + 2 minimum active balance quorum tests + 5 range sync/multi-epoch/payload attestation data integration tests + 5 broadcast_proposer_preferences VC tests + 3 broadcast_preferences error path tests + 2 produce_payload_attestations error path tests + 5 block replayer availability/blinded envelope edge case tests + 5 non-anchor block replayer envelope processing tests + 5 external bid payload withholding EMPTY-path chain continuation tests + 5 envelope processing error value/boundary/mutation tests + 5 envelope processing execution request path tests + 5 Gloas attestation gossip verification rejection tests + 5 withdrawal BLS credential/sweep wrap/multiple partials tests + 5 builder exit edge case tests + 5 Gloas aggregate attestation gossip verification tests + 5 process_payload_attestation edge case tests + 5 upgrade_to_gloas builder deposit edge case tests + 5 withdrawal processing interaction edge case tests + 5 Gloas signature set construction edge case tests + 5 builder pubkey cache correctness edge case tests + 5 deposit frontrunning edge case tests + 5 attestation participation flag Gloas edge case tests + 5 builder pending payments epoch rotation edge case tests + 5 withdrawal processing phase interaction edge case tests + 5 get_ptc_committee edge case tests + 5 find_head_gloas multi-block chain selection tests + 5 proto_array ptc_timely/reorg resistance/tiebreaker edge case tests + 5 fork choice on_execution_bid/on_payload_attestation/on_execution_payload integration tests + 5 process_epoch_single_pass Gloas builder payment/lookahead tests + 5 initiate_builder_exit lifecycle interaction tests + 5 process_deposit_requests batch behavior tests + 5 gossip verification untested error path tests + 3 GET payload attestation pool endpoint tests + 8 can_builder_cover_bid unit tests + 2 NotHighestValue/InactiveBuilder network gossip bid tests + 5 builder sweep early exit/zero builders/ineligible/reserved limit edge case tests + 1 abandoned fork envelope pruning integration test + 3 end-to-end builder payment accounting integration tests + 3 SSE event integration tests for Gloas ePBS gossip types + 1 external builder FULL-path chain continuation test + 3 PTC blob/payload quorum integration tests + 3 envelope execution request state-change tests + 1 block production after reorg attestation filtering test + 1 PTC duties epoch boundary integration test + 1 builder payment quorum promotion integration test + 2 prune_gloas_pools buffer cap tests + 1 duplicate envelope gossip dedup test + 1 past slot payload attestation gossip test + 1 envelope state transition failure after EL Valid test + 1 self-build envelope state transition failure after EL Valid test + 1 get_advanced_hot_state DB envelope re-application test + 2 proposer lookahead multi-epoch integration tests + 1 proposer_boost_root propagation regression test + 1 late bid envelope preservation test + 1 bid before envelope reset test + 7 fork choice quorum threshold/lifecycle/validation tests + 1 EMPTY-path reconstruction integration test + 1 process_pending_envelope EL Syncing path test + 1 proposer preferences head-behind-wall-clock gossip test + 1 empty-parent stale withdrawal persistence test + 1 ExecutionPayload SSE event gossip integration test + 1 gossip envelope EL Syncing optimistic path test + 1 gossip envelope EL Invalid fork choice skip test + 1 stale withdrawal carryover across EMPTY parent test + 1 gossip envelope InvalidSignature network handler test + 1 execution proof PriorToFinalization gossip handler test + 1 payload attestation ptc_weight accumulation network test + 1 pending envelope drain on gossip block import network test + 1 gossip block import no-envelope payload_revealed=false network test + 2 EL PayloadStatus::Accepted envelope processing tests + 2 gossip bid pool insertion integration tests + 1 reorg external bid filtering block production integration test + 1 Gloas block production HTTP API integration test + 3 builder voluntary exit gossip verification tests + 5 payload attestation pool retrieval/aggregation tests + 1 builder exit op pool retrieval test + 2 slashing op pool Gloas retrieval tests + 1 external builder envelope invalid signature HTTP API test + 2 block replayer execution request processing tests + 1 payload attestation unknown block root HTTP API test + 1 stale withdrawal mismatch envelope processing test + 4 execution proof gossip structural check tests + 1 envelope-before-block full gossip pipeline test + 1 range sync Fulu-to-Gloas fork boundary test + 1 prune_voluntary_exits OOB builder index retention test + 1 prune_gloas_pools slot boundary retention test + 1 bid submission invalid signature HTTP API test + 2 bid verification check ordering invariant tests + 6 process_payload_attestation block-level error path tests + 5 HTTP API bid submission error path tests + 5 remaining HTTP API bid error path tests + 1 multi-epoch mixed FULL/EMPTY chain finalization test + 8 ActivationQueue unit tests + 9 Balance unit tests + 5 BeaconCommittee unit tests + 10 BlobSidecar/BlobIdentifier unit tests + 8 SigningData/SignedRoot unit tests + 7 AttestationShufflingId unit tests + 3 NonZeroUsize unit tests + 14 EpochCache unit tests + 11 BeaconProposerCache unit tests + 23 slasher Config unit tests + 21 kzg TrustedSetup/G1Point/G2Point unit tests + 8 filesystem create_with_600_perms/restrict_file_permissions unit tests + 7 payload_cache unit tests + 4 keccak unit tests + 5 rayon_pool_provider unit tests + 7 unused_port unit tests + 21 KzgCommitment unit tests + 20 KzgProof unit tests + 35 batch state machine unit tests + 27 LRUTimeCache unit tests + 40 peer scoring unit tests + 36 peer_info connection/subnet/status unit tests + 46 ENR extension multiaddr/key conversion tests + 43 RPC methods types/conversion tests + 64 RPC protocol types/limits tests + 33 clap_utils parsing/SSZ/flag unit tests + 21 builder_client timeout/header/SSZ-tracking unit tests)
