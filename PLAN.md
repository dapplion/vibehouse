...
[docs/tasks/spec-update-alpha4.md](docs/tasks/spec-update-alpha4.md) — DONE

**alpha.3→alpha.4:** Audited all PRs in v1.7.0-alpha.4 release (Mar 24, 2026). No code changes needed — all behavioral changes already implemented (#5022 block-known check, #5008 field name). Remaining PRs are CI/tooling or unrelated features (EIP-8025).
...
Spec tests: [docs/tasks/spec-tests.md](docs/tasks/spec-tests.md) — DONE: 83/83 + 145/145 passing (v1.7.0-alpha.4 vectors, Heze SSZ static types, gossip validation tests for proposer/attester slashings and voluntary exits across all forks), automated release check workflow deployed
...
- **Heze fork transition** — DONE (script): `scripts/kurtosis-run.sh --heze` — Gloas@epoch1, Heze@epoch3, verified fork transition + finalization (epoch 8)
...
- **CI workflow** — DONE (`.github/workflows/ci.yml`: check, ef-tests, unit-tests, beacon-chain-tests, http-api-tests, network+op-pool-tests; `nightly-tests.yml`: all prior forks including gloas). CI now tests heze (latest fork) on every push, gloas moved to nightly matrix.
