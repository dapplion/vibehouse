...
this is `~/.vibehouse/{network}/validators/api-token.txt`, unless overridden using the
...
```bash
cat ~/.vibehouse/mainnet/validators/api-token.txt
...
```bash
curl http://localhost:5062/vibehouse/auth
```
...
```bash
curl localhost:5062/vibehouse/version -H "Authorization: Bearer hGut6B8uEujufDXSmZsT0thnxvdvKFBvh"
```

The server should respond with its version:

```json
{"data":{"version":"Vibehouse/v0.2.11-fc0654fbe+/x86_64-linux"}}
```
