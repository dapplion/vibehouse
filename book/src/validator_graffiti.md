...
Vibehouse provides four options for setting validator graffiti.
...
Usage:
`vibehouse vc --graffiti-file graffiti_file.txt`
...
Vibehouse will first search for the graffiti corresponding to the public key of the proposing validator, if there are no matches for the public key, then it uses the graffiti corresponding to the default key if present.
...
You can also update the graffitis in the `validator_definitions.yml` file using the [Vibehouse API](api_vc_endpoints.html#patch-vibehousevalidatorsvoting_pubkey). See example in [Set Graffiti via HTTP](#set-graffiti-via-http).
...
Usage: `vibehouse vc --graffiti example`
...
Usage: `vibehouse bn --graffiti fortytwo`
...
> 1. If the `--graffiti` flag is not passed, load the default Vibehouse graffiti.

## Set Graffiti via HTTP

Use the [Vibehouse API](api_vc_endpoints.md) to set graffiti on a per-validator basis. This method updates the graffiti
...
Refer to [Vibehouse API](api_vc_endpoints.html#patch-vibehousevalidatorsvoting_pubkey) for API specification.
