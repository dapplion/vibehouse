...
Since v3.4.0 Vibehouse will opportunistically re-org late blocks when proposing.

When Vibehouse is about to propose a new block, it quickly checks whether the block from the previous slot landed so late that hardly anyone attested to it.
If that late block looks weak enough, Vibehouse may decide to “re-org” it away: instead of building on it, Vibehouse builds its new block on the grand-parent block, turning the late block into an orphan.
...
* `--proposer-reorg-disallowed-offsets N1,N2,N3...`: Prohibit Vibehouse from attempting to reorg at
...
All flags should be applied to `vibehouse bn`. The default configuration is recommended as it
...
* Only single-slot re-orgs: Vibehouse will build a block at N + 1 to re-org N by building on the
  parent N - 1. The result is a chain with exactly one skipped slot.
* No epoch boundaries: to ensure that the selected proposer does not change, Vibehouse will
  not propose a re-orging block in the 0th slot of an epoch.

## Logs

You can track the reasons for re-orgs being attempted (or not) via Vibehouse's logs.
...
In case a re-org is not viable (which should be most of the time), Vibehouse will just propose a
...
when Vibehouse thinks that a re-org is likely:
