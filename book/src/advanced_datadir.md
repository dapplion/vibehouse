...
Users can override the default Vibehouse data directories (e.g., `~/.vibehouse/mainnet`) using the `--datadir` flag. The custom data directory mirrors the structure of any network specific default directory (e.g. `~/.vibehouse/mainnet`).
...
```bash
vibehouse --network mainnet --datadir /var/lib/my-custom-dir account validator import --directory <PATH-TO-LAUNCHPAD-KEYS-DIRECTORY>
vibehouse --network mainnet --datadir /var/lib/my-custom-dir bn --staking
vibehouse --network mainnet --datadir /var/lib/my-custom-dir vc
```
...
[#2682]: https://github.com/dapplion/vibehouse/pull/2682
[#2846]: https://github.com/dapplion/vibehouse/pull/2846

Prior to the introduction of [#2682][] and [#2846][] (releases v2.0.1 and earlier), Vibehouse would
not correctly parse relative paths from the `vibehouse bn --datadir` flag.
...
Vibehouse will continue to operate with split directories. In such a scenario, the following
...
1. Consolidating the legacy directory with the new one:
    ...
    - Where `$(pwd)` is the present working directory for the Vibehouse binary
