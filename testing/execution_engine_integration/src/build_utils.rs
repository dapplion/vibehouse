...
pub(crate) fn checkout(repo_dir: &Path, revision_or_branch: &str) -> Result<(), String> {
    output_to_result(
        Command::new("git")
            ...
            .map_err(|_| {
                format!(
                    "failed to checkout branch or revision at {}/{revision_or_branch}",
                    repo_dir.display(),
                )
            })?,
        |_| {},
    ...
    output_to_result(
        Command::new("git")
            ...
            .map_err(|_| {
                format!(
                    "failed to update submodules on branch or revision at {}/{revision_or_branch}",
                    repo_dir.display(),
                )
            })?,
        |_| {},
...
pub(crate) fn get_latest_release(repo_dir: &Path, branch_name: &str) -> Result<String, String> {
    ...
    output_to_result(
        Command::new("git")
            ...
            .current_dir(repo_dir)
            .output()
            .map_err(|e| format!("Failed to fetch tags for {}: Err: {e}", repo_dir.display()))?,
        |_| {},
    )?;
    output_to_result(
        Command::new("git")
            ...
            .map_err(|e| {
                format!(
                    "Failed to get latest tag for {}: Err: {e}",
                    repo_dir.display()
                )
            })?,
        |stdout| {
