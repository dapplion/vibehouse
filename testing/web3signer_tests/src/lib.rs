...
//! available on `$PATH`.
//!
//! This crate provides a series of integration tests between the Vibehouse `ValidatorStore` and
//! Web3Signer by Consensys.
//!
//! These tests aim to ensure that:
//!
//! - Vibehouse can issue valid requests to Web3Signer.
//! - The signatures generated by Web3Signer are identical to those which Vibehouse generates.
//!
...
mod tests {
    ...
    /// and Vibehouse agree on signatures.
    ///
    ...
    fn root_certificate_path() -> PathBuf {
        tls_dir().join("vibehouse").join("web3signer.pem")
    }

    fn client_identity_path() -> PathBuf {
        if cfg!(target_os = "macos") {
            tls_dir().join("vibehouse").join("key_legacy.p12")
        } else {
            tls_dir().join("vibehouse").join("key.p12")
        }
    }

    fn client_identity_password() -> String {
        fs::read_to_string(tls_dir().join("vibehouse").join("password.txt"))
            .unwrap()
            .trim()
            .to_string()
    }
    ...
    struct SlashingProtectionConfig {
        /// Whether to enable slashing protection for web3signer keys locally within Vibehouse.
        local: bool,
    }
    ...
    async fn test_vibehouse_slashing_protection(
        slashing_protection_config: SlashingProtectionConfig,
        listen_port: u16,
    ) {
        ...
        // protection is disabled in Vibehouse.
