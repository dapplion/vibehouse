...
mod gloas_per_block_tests {
    ...
    /// Build a minimal Gloas state for testing per-block processing functions.
    ...
    fn gloas_state_has_gloas_fork_name() {
        ...
        assert!(
            ...
            "Gloas state should have gloas_enabled() = true"
        );
    }

    #[test]
    fn is_merge_transition_complete_true_for_gloas() {
        let (state, _spec) = make_gloas_state();
        assert!(
