...
impl<E: EthSpec> ExecutionPayloadHeader<E> {
    ...
    pub fn ssz_max_var_len_for_fork(fork_name: ForkName) -> usize {
        // NOTE(newfork): Add a new case here if there are new variable fields
        ...
        D: Deserializer<'de>,
    {
        let convert_err = |e| {
            serde::de::Error::custom(format!(
                "ExecutionPayloadHeader failed to deserialize: {e:?}"
            ))
        };
        Ok(match context {
            ForkName::Base | ForkName::Altair => {
                return Err(serde::de::Error::custom(format!(
                    "ExecutionPayloadHeader failed to deserialize: unsupported fork '{context}'"
                )));
            }
            ForkName::Bellatrix => {
...
mod tests {
    ...
    fn make_fulu_header() -> ExecutionPayloadHeaderFulu<E> {
        ExecutionPayloadHeaderFulu {
            ...
            logs_bloom: FixedVector::new(vec![0x05; 256]).unwrap(),
            prev_randao: Hash256::repeat_byte(0x06),
            block_number: 42,
            gas_limit: 30_000_000,
            gas_used: 15_000_000,
            timestamp: 1_700_000_000,
            extra_data: VariableList::new(vec![0xAA, 0xBB]).unwrap(),
    ...
    fn make_gloas_payload() -> ExecutionPayloadGloas<E> {
        ExecutionPayloadGloas {
            ...
            logs_bloom: FixedVector::new(vec![0x15; 256]).unwrap(),
            prev_randao: Hash256::repeat_byte(0x16),
            block_number: 100,
            gas_limit: 60_000_000,
            gas_used: 30_000_000,
            timestamp: 1_800_000_000,
            extra_data: VariableList::new(vec![0xCC, 0xDD, 0xEE]).unwrap(),
