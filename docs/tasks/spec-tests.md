...
- **PR #4992 (PTC lookbehind)**: unchanged. 1 APPROVED (jtraglia Mar 12). Still OPEN.
- All other tracked PRs (#5002, #4954, #4843, #4898, #4892, #4939, #4940, #4932, #4960, #4962): all still OPEN.
- CI green (run 23035917712). 0 compatible dep updates. No code changes needed.
...
- CI run 23035917712 in_progress: check+clippy+fmt ✓, ef-tests ✓, network+op_pool ✓, http_api ✓, beacon_chain/unit tests still running.
- cargo audit unchanged (1 vuln + 5 allowed).
- No code changes needed.

### run 938 (Mar 13)
- Spec scan: no new consensus-specs merges since #5001 (Mar 12). No new spec release (still v1.7.0-alpha.2). No new spec-test vectors (still v1.6.0-beta.0).
- **PR #4992 (PTC lookbehind)**: unchanged (head d76a278b0a, clean). 1 APPROVED (jtraglia Mar 12). Still OPEN. Branch `ptc-lookbehind` ready.
- **PR #5002 (self-build wording)**: 1 APPROVED (jtraglia Mar 13). No consensus impact.
...
- Clippy clean (verified locally). cargo audit unchanged (1 vuln + 5 allowed). No compatible dep updates.
- No code changes needed.
