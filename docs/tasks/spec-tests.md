...
### run 1830 (Mar 18) — health check, all stable
...
- **Open Gloas PRs**: #4992 (cached PTCs) still open, mergeable=clean, active discussion — potuz pushing back on spec-level caching, ensi321 flagged `get_ptc` being too restrictive on slot range. #4747 (Fast Confirmation Rule) still open. None actionable.
- **Tests**: 4986/4986 workspace tests pass. Cargo audit unchanged.
- No code changes needed.

### run 1829 (Mar 18) — health check, all stable

- **CI**: nightly all green. All CI jobs green on HEAD.
- **Build**: `cargo check` clean (9s). `cargo clippy --workspace --all-targets` zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new Gloas PRs merged since #5005 (Mar 15).
- **Open Gloas PRs**: #4992 (cached PTCs) still open, mergeable=clean, active pushback from potuz on spec-level caching. #4747 (Fast Confirmation Rule) still open, dirty, 118 review comments. #4960, #4932 (test-only), #4840 (EIP-7843), #4630 (EIP-7688) all open, not actionable.
- **Tests**: 4986/4986 workspace tests pass. Cargo audit unchanged.
- No code changes needed.
...
- **Build**: `cargo check` clean (9s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new Gloas PRs merged since #5005 (Mar 15).
- **Open Gloas PRs**: #5015 (coverage restructure), #5014 (EIP-8025 p2p), #5012 (release-drafter), #5008 (doc field name fix), #4992 (cached PTCs), #4747 (Fast Confirmation Rule). None actionable.
- **Tests**: 4986/4986 workspace tests pass. 139/139 EF spec tests pass (fake_crypto, minimal).
- **Cargo audit**: unchanged — 1 known vulnerability (rsa, no fix), 5 allowed warnings.
- No code changes needed.
...
- **Build**: `cargo check` clean (9s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs — approved, still awaiting merge), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix — blocked), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests), #4840 (EIP-7843), #4630 (EIP-7688 SSZ).
- **PR #4992 status**: still open, approved. Will implement `previous_ptc`/`current_ptc` state fields + rotation in process_slots when merged.
...
### run 1803 (Mar 18) — health check, all stable

- **CI**: green on HEAD (`3d7553e`). All jobs pass.
- **Build**: `cargo check` clean (9s). `cargo clippy` clean. Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs — approved by jtraglia, close to merge), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix — blocked), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests), #4892 (remove impossible branch), #4898 (remove pending tiebreaker).
- **PR #4992 status**: approved, waiting on final merge. Will implement `previous_ptc`/`current_ptc` state fields + rotation in process_slots when merged.
- **Cargo audit**: unchanged (1 rsa vulnerability RUSTSEC-2023-0071, 5 allowed warnings).
- **Code quality scan**: ran clippy + codebase exploration. No actionable issues in production code. Test util unwraps are acceptable.
- **Spec test vectors**: still v1.6.0-beta.0 (no v1.7.0 vectors yet).
- No code changes needed.
...
- **Build**: `cargo check` clean (9s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests) — all still open/unmerged.
- No code changes needed.

### run 1800 (Mar 18) — health check, all stable

- **CI**: green on HEAD (`e7e7b60`). All jobs pass.
- **Build**: `cargo check` clean (9s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs, mergeable/clean — watch closely), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests), #4840 (EIP-7843), #4630 (EIP-7688 SSZ) — all still open/unmerged.
- **Spec test vectors**: still v1.6.0-beta.0 (no v1.7.0 vectors yet).
- No code changes needed.

### run 1799 (Mar 18) — health check, all stable

- **CI**: green on HEAD (`9dcfccd`). All jobs pass.
- **Build**: `cargo check` clean (<1s cached). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
...
### run 1798 (Mar 18) — health check, all stable

- **CI**: green on HEAD (`38825b5`). All jobs pass.
- **Build**: `cargo check --release` clean (17s). Zero warnings.
- **EF tests**: 139/139 pass (minimal, fake crypto).
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests), #4840 (EIP-7843), #4630 (EIP-7688 SSZ) — all still open/unmerged.
- **cargo audit**: unchanged (1 rsa vulnerability RUSTSEC-2023-0071, 5 allowed warnings).
- No code changes needed.
...
### run 1790 (Mar 17) — health check, all stable

- **CI**: all 7 jobs green on HEAD (`1e6aaff`). Nightly failure on stale commit `837cf89` (57 behind HEAD) — `finalized_sync_not_enough_custody_peers_on_start` passes locally on HEAD.
- **Build**: `cargo check --release` clean (17s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs), #4960 (fork choice deposit test), #4939 (missing envelope request), #5008 (field name fix), #4843 (variable PTC deadline), #4932 (sanity/blocks tests), #4962 (missed payload withdrawal tests), #4840 (EIP-7843), #4630 (EIP-7688 SSZ) — all still open/unmerged.
- **cargo audit**: unchanged (1 vulnerability rsa RUSTSEC-2023-0071, 5 allowed warnings).
...
### run 1788 (Mar 17) — health check, all stable

- **CI**: all 7 jobs green on HEAD (`1722042`). Latest run succeeded (33m).
- **Build**: `cargo check --release` clean (19s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new commits on master since #5005 (Mar 15). No new Gloas PRs merged.
- **Open Gloas PRs**: #4992 (cached PTCs), #4954 (fork choice ms), #5008 (field name fix), #4939 (missing envelope request), #4843 (variable PTC deadline), #4898 (remove pending tiebreaker), #4892 (remove impossible branch) — all still open/unmerged.
- **cargo audit**: unchanged (1 vulnerability rsa RUSTSEC-2023-0071, 5 allowed warnings).
- No code changes needed.
...
- **Build**: `cargo check --release` clean (17s). Zero warnings.
- **Spec**: v1.7.0-alpha.3 still latest release. No new Gloas PRs merged since last check.
- **Open Gloas PRs**: #4992 (cached PTCs), #4954 (fork choice ms), #5008 (field name fix), #4939, #4843 — all still open/unmerged.
- **cargo audit**: unchanged (1 vulnerability rsa RUSTSEC-2023-0071, 5 allowed warnings).
- No code changes needed.
