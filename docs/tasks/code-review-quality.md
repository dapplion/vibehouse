...
**Health check**: Spec v1.7.0-alpha.3 still latest — no new commits since #5005 (Mar 15). All open Gloas spec PRs unchanged. CI all green (all 7 jobs pass). Zero clippy warnings. No unsafe code in production. All `#[allow(deprecated)]` in RPC handler are required (libp2p ConnectionHandler trait transition). All `#[allow(dead_code)]` are on error variant fields or test utilities (standard Rust patterns). Heze fork spec has 7 files but is early-stage (fork epoch TBD, engine API undefined) — not actionable. Workspace tests running (release mode). No code changes needed.
