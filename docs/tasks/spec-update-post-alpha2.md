...
Notable activities:
- Run 852: Spec scan — all 11 tracked PRs still OPEN, no new merges, no new release (still v1.7.0-alpha.2). PR #4992 unchanged (commit 215962a), 8 reviews (all COMMENTED, no approvals). No compatible dep updates. Nightly Mar 10 failure was pre-fix commit 0d12a85 (data_column_reconstruction_at_deadline race); fix 62df5686 is on main, next nightly will be green. CI in progress for workflow_dispatch commit — check+clippy+fmt and ef-tests already passed.
- Run 850: Spec scan — all 11 tracked PRs still OPEN, no new merges, no new release (still v1.7.0-alpha.2). PR #4992 unchanged (commit 215962a), no new comments today. Recently merged consensus-specs PRs (#4984 remove EIP-6800, #4977 remove EIP-7441) — no consensus changes. No compatible dep updates. Nightly Mar 10 failure confirmed on pre-fix commit (ran before 62df5686); next nightly will be green. CI in progress for schannel update + workflow_dispatch commits. Added workflow_dispatch to CI for manual re-runs (run 849).
