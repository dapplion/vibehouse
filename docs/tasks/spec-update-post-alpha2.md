...
### run 1684 (Mar 17) — all stable, ptc-lookbehind rebased

Spec stable — no new consensus-specs commits since #5005 (Mar 15). v1.7.0-alpha.3 still latest release. PR #4992 still OPEN, NOT MERGED (same head d76a278b0a). No semver-compatible cargo updates. cargo audit unchanged (1 rsa). CI green. Nightly green. Rebased `ptc-lookbehind` branch onto main (38 commits behind → current). 1021/1021 state_processing tests pass. Pushed. All TODOs have issue links (issue #31 fully resolved). No code changes needed.

### run 1666 (Mar 17) — all stable, no code changes

Spec stable — no new consensus-specs commits since #5005 (Mar 15). v1.7.0-alpha.3 still latest release. PR #4992 still OPEN, NOT MERGED (same head d76a278b0a). No semver-compatible cargo updates. cargo audit unchanged (1 rsa). CI all green. Nightly green. No code changes needed.

### run 1650 (Mar 17) — all stable, no code changes

Spec stable — no new consensus-specs commits since #5005 (Mar 15). v1.7.0-alpha.3 still latest release. PR #4992 still OPEN, NOT MERGED (same head d76a278b0a). Active discussion: potuz questioning whether `get_ptc` is needed in spec at all — clients could just use `compute_ptc` for caching. Direction may simplify. No dep updates. cargo audit unchanged (1 rsa). CI all green. No code changes needed.

### run 1593 (Mar 16) — all stable, no code changes

Spec stable — no new consensus-specs commits since #5005 (Mar 15, test-only). v1.7.0-alpha.3 still latest release. No new spec-test vectors (still v1.6.0-beta.0). PR #4992 still OPEN, NOT MERGED (same head d76a278b0a, updated 7h ago with discussion activity). No semver-compatible cargo updates. cargo audit unchanged (1 rsa). CI green (nightly failure from earlier today confirmed as timing issue — slasher fix already on main). No code changes needed.

### run 1550 (Mar 16) — all stable, ptc-lookbehind rebased

Spec stable — no new consensus-specs commits since #5005 (Mar 15, test-only). v1.7.0-alpha.3 still latest release. No new spec-test vectors (still v1.6.0-beta.0). PR #4992 still OPEN, NOT MERGED (same head d76a278b0a). No semver-compatible cargo updates. cargo audit unchanged (1 rsa). CI green (nightly failure was stale — ran before slasher fix, next nightly will pass). Rebased `ptc-lookbehind` branch onto main. 1021/1021 state_processing tests pass. Pushed.
