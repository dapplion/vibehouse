...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2410, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs merged since run 2409. Nightly CI all green. Clippy clean (0 warnings). #4979 latest commits (e7b19104 "Fix epoch restrictions in paragraph", 89ce53b0 "Fix typo") are doc-only — no behavioral changes, implementation still aligned. #5036 latest commit (4e455a3) doc-only, still aligned. #4843 unchanged (mergeable: clean, approved). No action needed.
