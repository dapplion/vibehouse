...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2433, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs merged since run 2432. All open Gloas PRs unchanged. #4747 (FCR) still in active discussion (new comments today from @mkalinin, @etan-status on reorg metrics), mergeable_state=dirty, not close to merge. #4843 (variable PTC deadline) no new commits since Feb 19, still approved/open. #5036 (relax bid gossip) mergeable_state=blocked, unchanged. Build and clippy clean (zero warnings). cargo audit unchanged: 1 known vuln (rsa timing side-channel via jsonwebtoken, no fix available), 5 unmaintained warnings in transitive deps. CI green: all workflows passing. No action needed.
