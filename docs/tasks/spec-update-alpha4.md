...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2383, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new merged Gloas PRs since last check (run 2383). All open PRs unchanged — #4979 (PTC window) reopened after #4992 closed, still under discussion; #5036 and #4843 unchanged. CI passing (6/6 jobs). Clippy clean (zero warnings). Nightly tests green (Mar 24). No dependency updates available (only major version bumps). #4843 variable PTC deadline adds `payload_envelopes` to spec Store and `get_payload_size`/`get_payload_due_ms`/`is_payload_timely` helpers — vibehouse tracks equivalent functionality via BeaconChain envelope timing map (arrival_ms + ssz_size per block root).
