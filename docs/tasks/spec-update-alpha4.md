...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2369, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new merged Gloas PRs since last check (run 2369). CI all green. Clippy clean. cargo audit: only upstream RSA advisory (unfixable). All post-alpha.4 merged PRs (#5035, #5037, #4962, #4939, #5001, #5002) already implemented or not relevant. #5001 (`parent_block_root` in bid filtering key) verified — vibehouse already uses `(Slot, ExecutionBlockHash, Hash256)` tuple in `observed_execution_bids.rs`. #5002 is wording-only, no code change. #4979 PTC window implementation re-verified (run 2368) — no drift. Open PRs (#4979, #5036, #4843, #4747, #4960, #4932) all still open.
