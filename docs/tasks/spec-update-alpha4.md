...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2384, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new merged Gloas PRs since last check (run 2384). All open PRs unchanged — #4979 (PTC window) reopened after #4992 closed, still under discussion; #5036 and #4843 unchanged; #4747 (Fast Confirmation Rule) actively updated but still in design (merge conflicts, no approvals). CI passing (6/6 jobs). Clippy clean (zero warnings). Devnet health check passed (run 2384): finalized_epoch=8, 4 nodes, Gloas fork at epoch 1. No dependency updates available. cargo audit: 1 medium vuln (rsa/jsonwebtoken, no fix available), 5 unmaintained transitive deps (all in sp1-verifier or inherited code).
