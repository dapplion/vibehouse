...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2388, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new merged Gloas PRs since last check (run 2388). All open PRs unchanged. CI passing (6/6 jobs). Clippy clean (zero warnings, stable + nightly). Workspace tests: 4998/4998 pass (web3signer excluded, needs external service). cargo audit: 1 medium vuln (rsa/jsonwebtoken, no fix available). Dependencies fully current (only rand_xorshift 0.4→0.5 blocked by rand_core version mismatch). Heze fork: only 2 PRs (#4979 PTC window — proactively implemented, #4954 milliseconds — not worth proactive impl).
