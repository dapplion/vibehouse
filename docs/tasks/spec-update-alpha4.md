...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2437, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs merged since run 2436. All open Gloas PRs unchanged — #4747 (FCR) has new commit today (doc-only "State that update_fast_confirmation_variables must be called once"), still far from merge. Build and clippy clean (zero warnings). CI green: all workflows passing. No dependency updates available. cargo audit unchanged (1 rsa vuln, 5 unmaintained warnings). No action needed.
