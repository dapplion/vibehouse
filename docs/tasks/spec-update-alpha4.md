...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2390, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new merged Gloas PRs since last check (run 2390). All open PRs unchanged (#4979, #5036, #4843, #4747, #4960, #4932, #4898, #4892 all still OPEN). CI passing (6/6 jobs). Nightly passing (Mar 24 — slasher redb fix landed). Clippy clean (zero warnings). Workspace tests: 4998/4998 pass (web3signer excluded). EF tests: 140/140 pass (minimal+fake_crypto). cargo audit: 1 medium vuln (rsa/jsonwebtoken, no fix available). Heze fork: only 2 PRs (#4979 PTC window — proactively implemented, #4954 milliseconds — not worth proactive impl). No new consensus-specs PRs created since last check.
