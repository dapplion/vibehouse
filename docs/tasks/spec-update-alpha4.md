...
| #5044 | Speed up `compute_ptc` | Open (2026-03-26). Python perf optimization — pre-computes effective_balances list. **No vibehouse change needed** (logic already inlined). Blocking alpha.4 release. |
...
Run 2819 (2026-03-26): New PR opened: **#5044** "Speed up `compute_ptc`" (jtraglia, created 2026-03-26). Optimization to `compute_balance_weighted_acceptance` — changes signature from `(state, index, seed, i)` to `(effective_balance, seed, i)` by pre-computing effective balances list in `compute_balance_weighted_selection`. This is primarily a Python performance fix (test generation timed out at 12h for mainnet Gloas tests, blocking alpha.4 release). **No vibehouse code change needed** — our implementation already inlines the acceptance logic and accesses effective_balance via fast Vec indexing. v1.7.0-alpha.4 release blocked on this PR (test generation timeout). All other open Gloas PRs unchanged. CI green. No actionable work.

Runs 2820-2868 (2026-03-26): No new spec PRs merged since run 2819. #5044 still open (0 comments, 0 reviews — blocking alpha.4 release). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). #4747 (FCR) got CHANGES_REQUESTED from 0xMushow (Mar 26), still not close to merge. v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI green (ci, nightly, spec-test-check all passing Mar 26). Clippy zero warnings. cargo audit: 1 rsa vuln, 5 unmaintained. No actionable work.

Run 2869 (2026-03-26): No new spec PRs merged since run 2868. #5044 still open (blocking alpha.4). All open Gloas PRs unchanged. CI green. Fixed yanked unicode-segmentation 1.13.1→1.13.2 in Cargo.lock. cargo audit now shows 0 yanked warnings (1 rsa vuln, 5 unmaintained remain — all transitive, not actionable).

Run 2870 (2026-03-26): No new spec PRs merged since run 2869. #5044 still open (0 comments, 0 reviews — blocking alpha.4). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI in progress for unicode-segmentation update (run 23603229624). No actionable work.

Run 2871 (2026-03-26): No new spec PRs merged since run 2870. #5044 still open (0 comments, 0 reviews — blocking alpha.4 release). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI in progress for unicode-segmentation fix. cargo audit: 1 rsa vuln, 5 unmaintained (all transitive). cargo outdated: only rand_xorshift 0.4→0.5 (test utility). No actionable work.

Run 2872-2873 (2026-03-26): No new spec PRs merged. #5044 still open (0 reviews, blocking alpha.4 release). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). v1.7.0-alpha.4 release still not published. EF test vectors still v1.6.0-beta.0. CI: check+clippy pass, tests in progress for unicode-segmentation fix. cargo audit: 1 rsa vuln (no fix), 5 unmaintained (all transitive). No actionable work.

Runs 2874-2878 (2026-03-26): No new spec PRs merged since run 2873. #5044 still open (0 comments, 0 reviews — blocking alpha.4 release). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI: nightly green, ci in progress for unicode-segmentation 1.13.2 fix (check+clippy+ef-tests+network+op_pool passed, 3 jobs remaining). No actionable work.

Runs 2879-2885 (2026-03-26): No new spec PRs merged since run 2878. New PR #5045 opened (remove @always_bls decorator — test infra only, not relevant). #5044 still open (0 comments, 0 reviews, mergeable_state=blocked — blocking alpha.4 release). All open Gloas PRs unchanged (#4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630, #5044). v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI for compute_ptc optimization: check+clippy+ef-tests+network+op_pool passed, http_api+unit+beacon_chain still running. Zero compiler warnings. All remaining TODOs blocked on external deps (EIP-7892 ×3, blst, PeerDAS, lmdb, async locks). No actionable work.
