...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2401, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs since last run. #4979 latest commits (e7b19104, 89ce53b0) are docs/typo only — implementation still aligned. Nightly CI run 23533380698: 24/26 jobs green, http-api fulu/electra still running. `cargo audit`: unchanged. No action needed this run.
