...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2419, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs merged since run 2418. Nightly CI all green. Clippy clean. `cargo audit`: rsa vulnerability (RUSTSEC-2023-0071, no fix available upstream — via jsonwebtoken) + known unmaintained crate warnings (ansi_term, bincode, derivative, paste). All open Gloas PRs unchanged. No action needed.
