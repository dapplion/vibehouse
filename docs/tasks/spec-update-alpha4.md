...
No v1.7.0-alpha.4 release/tag created yet on consensus-specs (as of run 2418, 2026-03-25). Version bump PR (#5034) merged Mar 24 but no GitHub release published. Spec-test-check workflow will auto-detect when it's published. Current pinned version: v1.7.0-alpha.3. EF test vectors also not updated (latest: v1.6.0-beta.0 from Sep 2025). No new Gloas PRs merged since run 2417. Nightly CI all green (26/26 jobs). Clippy clean. `cargo audit`: only known unmaintained crate warnings (rsa, ansi_term, bincode, derivative, paste) — no actionable fixes. All open Gloas PRs unchanged — #4979 latest commit (89ce53b, Mar 24) is typo fix only, #5036 latest (4e455a3) is doc-only. No action needed.
