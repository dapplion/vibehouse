...
Run 2464: No new spec PRs merged since run 2463. All open Gloas PRs unchanged (#4979, #5036, #4843, #4898, #4892, #4960, #4932, #4954). #4979 still 15 commits (last: 89ce53b0 Mar 24), mergeable:clean. v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0 (Sep 2025). CI in progress for run 2463 http_api fix (all 6 jobs running). No actionable work.

Run 2465: No new spec PRs merged since run 2464. All open Gloas PRs unchanged (#4979, #5036, #4843, #4898, #4892, #4960, #4932, #4954). v1.7.0-alpha.4 release still not published (latest release = v1.6.1). EF test vectors still v1.6.0-beta.0. CI in progress for run 2463 http_api fix (check+clippy passed, 5 test jobs running). Nightly green (Mar 24+25). cargo audit: same known advisories (1 rsa vuln, 5 unmaintained). No actionable work.

Run 2466: No new spec PRs merged since run 2465. All open Gloas PRs unchanged (#4979, #5036, #4843, #4898, #4892, #4960, #4932, #4954). #4979 still 15 commits (last: 89ce53b0 Mar 24), mergeable:clean. #4747 updated today (79 commits). v1.7.0-alpha.4 release still not published (latest = v1.7.0-alpha.3). EF test vectors still v1.6.0-beta.0. CI in progress for run 2463 http_api fix (check+clippy+ef passed, 4 jobs still running). Clippy: zero warnings. cargo audit: same known advisories. No actionable work.
...
| ~~#4558~~ | ~~Add Cell Dissemination via Partial Message Specification~~ | **MERGED 2026-03-25.** Adds Gloas `PartialDataColumnHeader` (removes `signed_block_header`/`kzg_commitments_inclusion_proof`, adds `slot`/`beacon_block_root`) and partial message validation rules on `data_column_sidecar_{subnet_id}`. **Not actionable** — requires Gossipsub partial messages extension (libp2p spec not yet finalized, no Rust libp2p implementation). Will implement when libp2p support lands. |
...
Run 2478: **#4558 merged** ("Add Cell Dissemination via Partial Message Specification", 2026-03-25). Adds Gloas-modified `PartialDataColumnHeader` container and partial message validation rules for `data_column_sidecar_{subnet_id}`. **Not actionable** — requires Gossipsub partial messages extension which has no Rust libp2p implementation yet. Remaining open Gloas PRs unchanged (#5036, #4843, #4898, #4892, #4960, #4932, #4954, #4747, #4840, #4630). v1.7.0-alpha.4 release still not published (latest = v1.6.1). EF test vectors still v1.5.0. CI fully green. Clippy: zero warnings. No actionable work.
