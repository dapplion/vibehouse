...
impl<T: BeaconChainTypes> BeaconChain<T> {
    ...
    pub async fn process_payload_envelope(
        ...
        let result = self.process_payload_envelope_inner(verified_envelope).await;
        if result.is_err() {
            metrics::inc_counter(&metrics::PAYLOAD_ENVELOPE_PROCESSING_FAILURES);
        }
        result
    }

    async fn process_payload_envelope_inner(
        &self,
        verified_envelope: &crate::gloas_verification::VerifiedPayloadEnvelope<T>,
    ) -> Result<bool, Error> {
        ...
        // for external builders and self-build alike).
    ...
    pub async fn process_self_build_envelope(
        &self,
    ...
    async fn check_gossip_data_columns_availability_and_import(
        ...
        if let Some(slasher) = self.slasher.as_ref() {
            for data_colum in &data_columns {
                if let Some(header) = data_colum.signed_block_header() {
    ...
    fn import_block(
        ...
        if self.config.enable_light_client_server {
            self.light_client_server_cache
                .cache_state_data(&self.spec, block, block_root, &state)
                .unwrap_or_else(|e| {
