...
pub(crate) const DEFAULT_PAYLOAD_CACHE_SIZE: NonZeroUsize = new_non_zero_usize(10);

/// A cache mapping execution payloads by tree hash roots.
pub(crate) struct PayloadCache<E: EthSpec> {
...
impl<E: EthSpec> PayloadCache<E> {
    pub(crate) fn put(&self, payload: FullPayloadContents<E>) -> Option<FullPayloadContents<E>> {
        let root = payload.payload_ref().tree_hash_root();
        self.payloads.lock().put(PayloadCacheId(root), payload)
    }

    #[cfg(test)]
    pub(crate) fn pop(&self, root: &Hash256) -> Option<FullPayloadContents<E>> {
        self.payloads.lock().pop(&PayloadCacheId(*root))
    }

    pub(crate) fn get(&self, hash: &Hash256) -> Option<FullPayloadContents<E>> {
