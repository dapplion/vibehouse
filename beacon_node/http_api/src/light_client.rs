...
pub fn validate_light_client_updates_request<T: BeaconChainTypes>(
    ...
    if query.count > MAX_REQUEST_LIGHT_CLIENT_UPDATES {
        return Err(ApiError::bad_request("Invalid count requested".to_string()));
    }

    let current_sync_period = chain
        .epoch()
        .map_err(|_| ApiError::server_error("failed to get current epoch".to_string()))?
        .sync_committee_period(&chain.spec)
        .map_err(|_| {
            ApiError::server_error("failed to get current sync committee period".to_string())
        })?;
