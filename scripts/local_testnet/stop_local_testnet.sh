...
# Extract the service names of Vibehouse beacon nodes that start with "cl-".
...
for service in $services; do
  kurtosis files storeservice --name "$service-logs" "$ENCLAVE_NAME" "$service" /data/vibehouse/beacon-data/beacon/logs/
done
...
echo "The vibehouse beacon nodes' logs (including dependency logs) can be found in $LOGS_SUBDIR/files/cl-*-lighthouse-geth-logs."
